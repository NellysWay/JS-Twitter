import { getUser, deletePost, updatePost } from "./ajax.js";
import { currentUser, displayUserOnPost } from "./users.js";


export function displayPosts(posts) {
    const newsfeedSection = document.querySelector("#main-feed");
    newsfeedSection.innerHTML = "";
    posts.forEach(post => newsfeedSection.innerHTML += `
    <section class="post">
            <div class="pic">
                <h6 class="name" id="username${post.id}">@${getUser(post.userId, displayUserOnPost, post.id)}</h6>
            </div>
    ...
    newsfeedSection.addEventListener('click', function(event) {
        if(event.target && event.target.matches(".edit-btn")) {
            ...
            const bodyText = card.querySelector(".chat-body p").innerHTML;
            const cardBody = card.querySelector(".chat-body");
